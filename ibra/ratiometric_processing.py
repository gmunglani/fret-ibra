...
import cv2
from functions import h5, logit, time_evolution, tiff, bleach_fit, ratio_calc, background_intensities
...
def bleach(verbose, logger, work_out_path, acceptor_bound, donor_bound, fitter, h5_save, tiff_save, frange):
    ...
    if (acceptor_bound[1] > acceptor_bound[0]):
        ...
        assert (sum(~np.isin(acceptor_bound,
                             ratio_frange)) == 0), "acceptor_bleach_range should be within processed frame range"
        ...
        acceptoro = copy.deepcopy(acceptori)
    ...
    if (donor_bound[1] > donor_bound[0]):
        ...
        assert (sum(
            ~np.isin(donor_bound, ratio_frange)) == 0), "donor_bleach_range should be within processed frame range"
    ...
    # Update log file
    logger.info('(Bleach Correction) ' + 'acceptor_bleach_frames: ' + str(acceptor_bound[0] + 1) + '-' + str(
        ratio_frange[-1] + 1)
    ...
    time_evolution(acceptori, donori, work_out_path, '_intensity_bleach.png', 'Median Intensity/Bit Depth',
                   h5_save=False)

    background_intensities(acceptoro, acceptori, donori, work_out_path)
    ...
    if (h5_save or tiff_save):
        ...
        if (h5_save):
            ...
            if (verbose):
                print("Saving Ratio stack in " + work_out_path + '_back_ratio.h5' + ' [Time: ' + str(
                    int(h5_time_end - h5_time_start) + 1) + " second(s)]")
        ...
        if (tiff_save):
            ...
            if (verbose):
                print(
                    "Saving bleached Ratio TIFF stack in " + work_out_path + '_back_ratio_bleach.tif' + ' [Time: ' + str(
                        int(tiff_time_end - tiff_time_start) + 1) + " second(s)]")
...
def ratio(verbose, logger, work_out_path, crop, res, register, union, h5_save, tiff_save, frange):
    ...
    assert (sum(~np.isin(frange,
                         acceptor_frange)) == 0), "background subtracted stacks have not been processed for all frame values"
    ...
    for count, frame in list(zip(frange, brange)):
        ...
        # Otsu thresholding
        _, A_thresh = cv2.threshold(np.uint8(np.float16(acceptorc[frame, :, :]) * mult), 0, 255,
                                    cv2.THRESH_BINARY + cv2.THRESH_OTSU)
        _, B_thresh = cv2.threshold(np.uint8(np.float16(donorc[frame, :, :]) * mult), 0, 255,
                                    cv2.THRESH_BINARY + cv2.THRESH_OTSU)
    ...
    if (max(np.ediff1d(frange, to_begin=frange[0])) > 1):
        logger.info('(Ratio Processing) ' + 'frames: ' + ",".join(
            list(map(str, print_range))) + ', time: ' + time_elapsed + ' sec, save: ' + str(h5_save))
    else:
        logger.info('(Ratio Processing) ' + 'frames: ' + str(print_range[0]) + '-' + str(
            print_range[-1]) + ', time: ' + time_elapsed + ' sec, save: ' + str(h5_save))
    ...
    if (h5_save or tiff_save):
        ...
        if (h5_save):
            ...
            if (verbose):
                print(("Saving Acceptor, Donor and Ratio stacks in " + work_out_path + '_back_ratio.h5' + ' [Time: ' + str(
                              int(h5_time_end - h5_time_start) + 1) + " second(s)]"))
        ...
        if (tiff_save):
            ...
            if (verbose):
                print(("Saving unbleached Ratio TIFF stack in " + work_out_path + '_back_ratio.tif' + ' [Time: ' + str(
                    int(tiff_time_end - tiff_time_start) + 1) + " second(s)]"))
